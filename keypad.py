...
class SyntheticButton():
    def __init__(self, label):
        self._value = 0
        self.label = label
        self._when_pressed = lambda: print(f"{label} pressed")
...
class Pad:
    ...
    def __init__(self, 
        ...
        self._buttons = [[SyntheticButton(self._labels[row][col]) for col in range(len(self._cols))] for row in range(len(self._rows))]
